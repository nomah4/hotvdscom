...
## Аккаунт `382713856310378498` — демо владельца установки, а не клиент

Записано, чтобы никто не разбирал его как клиентский. На нём четыре действующие подписки
(`Demo test`, `VDS Victor` и две импортированные машины со сроком до 2027 года) и шесть отозванных
11–12 августа с оплаченными счетами на 8 040 ₽, по которым **нет ни одного возврата в PO**. Это
внутренняя уборка после переезда на боевое железо, а не долг перед клиентом.

Из выручки этот аккаунт исключать. Платящие на 15 августа — `385104192777224194`,
`385040983357652994`, `385227521706164226` и `386254842198228994`.
...
- [ ] **3 `payment_state_mismatch`** — открыты 20 июля и 10 августа по платежам на 4400, 4400 и
      900 ₽. **Проверено 15 августа: дырки в PO нет.** Все три платежа получили состояние `failed`
      в тот же день, когда были созданы, и незавершённых платежей в PO сейчас нет ни одного.
      Кейсы описывают снимок, сделанный в момент, когда платёж был `pending` секунды или минуты
- [ ] **Сверка открывает кейсы и никогда их не закрывает.** `reconcile_payments` только считает и
      создаёт (`workers.py`), а единственное место, где кейс становится `resolved`, — действие
      администратора (`views.py:1341`). Условие, отпавшее через пять минут, оставляет запись
      навсегда. Нужен проход «условие ещё выполняется?» по открытым кейсам — иначе очередь
      наполняется мусором, в котором тонет настоящее. Сегодняшний разбор ровно так и выглядел:
      36 открытых кейсов, из которых актуальных не осталось ни одного
