...
**За истёкшую услугу снова можно заплатить.** В день, когда срок выходил, кабинет переставал брать
деньги: чип «Действует до» — это и есть кнопка продления, а на не-active подписке он вырождался в
обычный серый текст. Заплатить за свой сервер клиенту было негде, и обе его услуги висели `Expired`
без единого действия на карточке.

Причина лежала в биллинге, а не в витрине: продление отвергалось для любого статуса кроме `active`
(422 `subscription_not_renewable`). Совет «купить заново», записанный там же, никуда не годился —
под политикой `separate` покупка заводит новую подписку и новую пустую машину, а оплаченный диск
остаётся выключенным под старой. Теперь продление открыто для `active`, `past_due` и `expired`;
`cancelled` и `revoked` по-прежнему нельзя — за ними нет ни машины, ни прав.

### Changed

**`Expired` и «Действует до» горят красным.** Серая точка сообщала ровно то же, что у слова
«Остановлен», хотя в первом случае от клиента ждут денег. Красный — единственный тон на карточке,
который о чём-то просит.

**Рядом с датой появилась кнопка «Оплатить».** У живой услуги всё как было: дата сама и есть кнопка
продления, потому что дата и её продление — одна мысль. У истёкшей дата перестаёт быть кнопкой —
прошедший срок это не обещание, по которому кликают, а причина, по которой сервер лежит, — и глагол
переезжает на отдельную кнопку.

**Истёкшая услуга осталась в «Моих серверах», а не уехала в «Историю».** Раздел истории появился
накануне и забрал `expired` вместе с `revoked` и `cancelled`. Но истёкшую услугу оплачивают, и на её
карточке стоит единственная кнопка, ради которой в кабинет приходят с деньгами; свёрнутый раздел —
последнее место, где её станут искать. В историю уходит то, что кончилось насовсем.

**Кабинет больше не решает за биллинг, что можно продлить.** Флаг `renewable` приезжал в ответе
`renewal-preview` с самого начала и не читался; витрина повторяла правило у себя. Теперь правило
одно и живёт на сервере, а окно подтверждения гасит кнопку оплаты, если сервер говорит «нельзя», —
вместо отказа после нажатия.

## 2026-08-18

### Added

**Кнопка «Сменить IP» на карточке сервера.** Клиент меняет белый адрес своей машины сам, не
обращаясь в поддержку. Работает по всей цепочке — витрина, `bl`, `pr`, движок, — потому что ни один
из вопросов, которые тут возникают, браузеру не по силам: какой адрес свободен, знает пул локации, а
лимит, проверяемый в браузере, — это не лимит.

**Окно подтверждения показывает конкретный адрес, а не спрашивает «уверены?».** Смена ломает всё,
что указывает на старый адрес: записи DNS, белые списки, чужие фаерволы. Новый адрес набран крупно
и выделяется одним щелчком — его переписывают. Движок придерживает его за клиентом на четверть часа,
поэтому подтверждают ровно то, что показано.

Там же сказано, что машина перезагрузится: адрес поднимается на следующей загрузке, и узнать об этом
после нажатия поздно. Если перезагрузить не вышло или машина выключена, карточка говорит прямо, что
перезагрузить придётся самому, — иначе клиент ждал бы адрес, который поднимут только его руки.

**Лимит — раз в неделю, и он виден до нажатия.** Биллинг присылает `ip_change_allowed_at` вместе с
машиной; пока неделя не прошла, кнопка на месте, не нажимается и в подсказке называет день.
Исчезнувшая кнопка читается как поломка, а отказ в ответ на подтверждённое действие — тем более.

Кнопки нет вовсе, пока биллинг не отдаёт этот ключ: как и с переименованием, отсутствие ключа
означает «эндпоинта нет», и включённая против него кнопка отвечала бы отказом на каждое нажатие.

**Выкачено на прод в тот же день, тремя слоями в порядке зависимостей**: движок (миграция `0011` —
карантин адреса, бронь и отметка последней смены), затем `bl` и `pr` тегом `v0.3.0`, затем витрина.
Порядок не косметический: витрина, выехавшая первой, просто не показала бы кнопку, а `bl`,
выехавший раньше движка, ретранслировал бы действие в эндпоинт, которого ещё нет. На момент выката
в пуле Амстердама 157 свободных адресов из 184.
