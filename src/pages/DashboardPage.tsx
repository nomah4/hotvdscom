...
/**
 * Что остаётся в «Моих серверах», а что уезжает в историю.
 *
 * `expired` здесь, а не в истории, и это главное в этом наборе: истёкшую услугу
 * можно оплатить, и на её карточке стоит единственная кнопка, ради которой
 * клиент вообще приходит в кабинет с деньгами. Свёрнутый раздел «История» —
 * последнее место, где её станут искать; спрятать туда услугу, которая ждёт
 * оплаты, значит спрятать сам счёт.
 *
 * В историю уходит то, что кончилось насовсем: `revoked` и `cancelled`. За ними
 * нет ни машины, ни возможности заплатить — только запись о том, что было.
 */
